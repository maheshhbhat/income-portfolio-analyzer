# Maximum withdrawal Chrome UAT record

This is an owner-facing Chrome acceptance record for Story #51. It deliberately
uses the **Illustrative comparison** data set; the maximum withdrawal
calculator must not use a provider snapshot or Refresh Data.

## Recorded run procedure

1. Start the application with `npm start`, then open the displayed local URL in
   Google Chrome. Open DevTools and select the Console panel; enable Preserve
   log and clear existing messages.
2. Leave **Fund provider** set to **Illustrative comparison**. In **Maximum
   sustainable withdrawal**, enter `$10.00`, `10` years, and `2` percent
   inflation, then select **Calculate maximum withdrawal**.
3. Verify a visible dollar result and text saying it is the “highest verified
   first-year annual withdrawal” and that increasing it by `$0.01 fails`.
   Verify that the supporting allocation table has one or more rows, the
   projection has ten rows, and the disclosure says the result is illustrative,
   not financial advice, not a forecast, and not a certainty.
4. Replace the starting portfolio with `0` and calculate again. Verify the
   prior result, allocation, and projection are hidden; an actionable starting
   portfolio error is shown; and no withdrawal amount is displayed.
5. Inspect the Console. Record pass only when there are no errors generated by
   the page during either submission (ignore messages emitted by browser
   extensions, if any).

## Execution record

The owner completed the run on 2026-08-26 in Google Chrome 151.0.7922.174
against exact PR #57 head
`baae6b883280589f30b9c9b39e0643db42d7a399`. The application was served at
`http://127.0.0.1:3011/`. This is owner UAT evidence for Story #51; it is not a
Project acceptance decision.

Later commits in PR #57 recorded or tightened acceptance evidence and automated
tests. Per the Story #51 acceptance notes, those later commits did not change
the served application or runtime behavior exercised in this Chrome run.

| Field | Record |
| --- | --- |
| Run at | 2026-08-26 (America/New_York) |
| Chrome | Google Chrome 151.0.7922.174 |
| Local URL | `http://127.0.0.1:3011/` |
| Owner-verified application head | `baae6b883280589f30b9c9b39e0643db42d7a399` |
| Later PR work after owner run | Acceptance-evidence and automated-test updates only; no served-app/runtime change per Story #51 acceptance notes |
| Provider | Illustrative comparison |
| Successful result | A $10.00 starting portfolio, 10-year horizon, and 2% inflation rendered a verified maximum of $1.58. The visible proof said this was the highest verified first-year annual withdrawal and that $0.01 more fails. |
| Supporting result | The page rendered an 11.12% blended return, five allocation rows, and ten projection rows. |
| Disclosure | Visible text said the result is illustrative, not financial advice, not a forecast, and not a certainty. |
| Invalid-input behavior | Replacing the starting portfolio with 0 hid the prior result, rendered zero allocation and projection rows, displayed no withdrawal figure, and showed: “Enter a starting portfolio as a safe whole number of cents greater than $0.” |
| Page-console errors or warnings | 0 |
| Durable owner evidence | [Story #51 owner Chrome UAT evidence](https://github.com/maheshhbhat/income-portfolio-analyzer/issues/51#issuecomment-5424581733) |

The automated companion is `test/regressionMaxWithdrawal.test.js`; it proves
the cent boundary, non-monotone bracket case, refusal behavior, determinism,
and the curated-only/provider-independent binding before the manual Chrome
rendering check.
